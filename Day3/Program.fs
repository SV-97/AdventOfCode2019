...
type Position =
    { X: int
      Y: int }
    override pos.ToString() = sprintf "(%d, %d)" pos.X pos.Y
...
type Wire = Position list

/// move from one position to another - also yielding all intermediary positions
/// The last element in the Output is the final position
let move (pos: Position) (movement: Movement): Position list =
    match movement with
    | Up y ->
        [ for i in 1 .. y do
            yield { pos with Y = pos.Y + i } ]
    | Down y ->
        [ for i in 1 .. y do
            yield { pos with Y = pos.Y - i } ]
    | Left x ->
        [ for i in 1 .. x do
            yield { pos with X = pos.X - i } ]
    | Right x ->
        [ for i in 1 .. x do
            yield { pos with X = pos.X + i } ]

/// find all crossings of two wires
let crossings (wire1: Wire) (wire2: Wire): Position list =
    let poss1 = Set.ofList wire1
    let poss2 = Set.ofList wire2
    Set.intersect poss1 poss2 |> Set.toList

let manhattanDistance ({ X = x; Y = y }: Position) = abs (x) + abs (y)

let toWire (movements: Movement list): Wire =
    (({ X = 0
        Y = 0 }, []), movements)
    ||> List.fold (fun (currentPos, wire) movement ->
            let wireSegment = move currentPos movement
            let newPos = List.rev wireSegment |> List.head
            (newPos, wire @ wireSegment))
    |> fun (currentPos, wire) -> wire
//|> List.rev
//|> List.tail

let parseMovements (raw: String): Movement list =
    raw.Split [| ',' |]
    |> Array.map (fun segment ->
        ...
        |> match segment.[0] with
           ...
           | 'R' -> Right
           | 'L' -> Left
           | c -> raise (ArgumentException(sprintf "invalid char %c" c)))
    |> Array.toList

// Couldn't get NUnit installed 🤷‍
let test(): bool =
    let raw1 = "R8,U5,L5,D3"
    let raw2 = "U7,R6,D4,L4"
    let wire1 = parseMovements raw1 |> toWire
    let wire2 = parseMovements raw2 |> toWire
    let crossings = (crossings wire1 wire2)
    let leastDistance = (List.map manhattanDistance crossings |> List.min)
    if leastDistance <> 6 then
        printfn "Wire #1:"
        List.map (printf "%O ") wire1 |> ignore
        printfn ""
        printfn "Wire #2:"
        List.map (printf "%O ") wire2 |> ignore
        printfn ""
        printfn "Crossings:"
        List.map (printf "%O ") |> ignore
        printfn ""
    leastDistance = 6
...
let main argv =
    if test() then
        ...
        printfn "X=%d, Y=%d, Distance=%d" closestCrossing.X closestCrossing.Y (manhattanDistance closestCrossing)
